...
void Kernel::ExecAll()
{
    ...
    // This is declared in kernel.h ----> "char*  execfile[10];" yyz20171114
...
{
    // This is declared in kernel.h ----> " Thread* t[10]; " yyz20171114
...
int Kernel::Open(char *name)
{
    ...
    OpenFile* ptr = fileSystem->Open(name);
    int id = (int)ptr;
    return id; // YYZ20171013
 }

int Kernel::Write(char* buffer, int ch_size, int id)
{
    bool exist = false;
    int i;
    for (i = 0; i < 20; ++i)
    {
        OpenFile* temp = fileSystem->fileDescriptorTable[i];
        int content = (int)temp;
        if (content == id)
        {
            exist = true;
            break;
    ...
    if (exist == true)
        return fileSystem->fileDescriptorTable[i]->Write(buffer, ch_size);
    return -1;
}

int Kernel::Read(char* buffer, int ch_size, int id)
{
    bool exist = false;
    int i;
    for (i = 0; i < 20; ++i)
    ...
    if (exist == true)
        return fileSystem->fileDescriptorTable[i]->Read(buffer, ch_size);
    return -1;
    //return fileSystem->fileDescriptorTable[id]->ReadAt(buffer, ch_size, id);
}
...
void Kernel::PrintInt(int number) // prints integer
{
    char ch;
    if (number < 0)
    {
